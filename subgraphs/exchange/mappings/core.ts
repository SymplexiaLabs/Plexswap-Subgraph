...
import { updatePairDayData, updateTokenDayData, updatePlexDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSwap(event: Swap): void {
  ...
  let plexDayData = updatePlexDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  plexDayData.dailyVolumeUSD = plexDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  plexDayData.dailyVolumeBNB = plexDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  plexDayData.dailyVolumeUntracked = plexDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  plexDayData.save();
